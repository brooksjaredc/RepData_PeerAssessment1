...
```{r histogramAndAverage, message=FALSE}
library(dplyr)
...
The mean number of steps per day is `r meanperday` and the median is `r medianperday`
...
The maximum number of steps averaged across all days occurs in interval number `r max_steps_interval`.
...
For the dataset with imputed values, the mean number of steps per day is `r meanperday.imputed` and the median is `r medianperday.imputed`.  
